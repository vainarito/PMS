...
public class CartActivity extends AppCompatActivity {
    ...
    public native double[] calculateCart(double totalFee, boolean isCartEmpty);
    ...
    @SuppressLint("SetTextI18n")
    private void calculatorCard() {
        // Вызов нативного метода для расчета данных корзины
        double[] results = calculateCart(managmentCart.getTotalFee(), managmentCart.getListCart().isEmpty());

        double itemTotal = results[0];
        double tax = results[1];
        double delivery = results[2];
        double total = results[3];
